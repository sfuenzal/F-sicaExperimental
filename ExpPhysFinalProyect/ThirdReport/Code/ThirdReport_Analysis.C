            ...
            for(UInt_t entry = 0; entry < numberOfEntries; ++entry)
            {
                ...
                if(branchParticle -> GetEntries() > 0 && branchJet -> GetEntries() > 0)
                {
                    ...
                    for(UInt_t i = 0; i < branchParticle -> GetEntries(); ++i)
			        {
                        ...
                        {
                            lep_pt.push_back(particle -> PT);
                            lep_eta.push_back(particle -> Eta);
                            lep_phi.push_back(particle -> Phi);
                            lep_E.push_back(particle -> E);
                            lep_charge.push_back(particle -> Charge);
                            lep_type.push_back(particle -> PID);
